...
reporter_del = 10  # переменная, чтобы можно было изменить объем запроса
###########################################################################
...
reporter = reporter.drop_duplicates()
...
result = result.sort_values(by=['reporter', 'code'])  # Важно! Cнижает расхождения данных по стране
...
request_passport_id = OrmComtradeRequestsInsert(request_passport, None)  # получить паспорт
...
for i in index_result:
    ...
    """
    OrmComtradeRequestsInsert(request, request_passport_id)
    print(
